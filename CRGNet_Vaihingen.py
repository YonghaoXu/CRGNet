...
def get_arguments():
    ...
    parser.add_argument("--data_dir", type=str, default='/iarai/home/yonghao.xu/Data/Vaihingen/',
                        help="dataset path.")
    parser.add_argument("--train_list", type=str, default='./dataset/vaihingen_train.txt',
                        help="training list file.")
    parser.add_argument("--test_list", type=str, default='./dataset/vaihingen_test.txt',
                        help="test list file.")
...
def main():
    ...
    args = get_arguments()
    snapshot_dir = args.snapshot_dir+'Vaihingen/CRGNet_mode_'+modename[args.mode]+'_id_'+str(args.id)+'/time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    if os.path.exists(snapshot_dir)==False:
        os.makedirs(snapshot_dir)
    f = open(snapshot_dir+'VaihingenSeg_log.txt', 'w')
    ...
    src_loader = data.DataLoader(
                    VaihingenDataSet(args.data_dir, args.train_list, max_iters=args.num_steps_stop*args.batch_size,
    ...
    for batch_index, src_data in enumerate(src_loader):
        ...
        if (batch_index+1) % 100 == 0:            
            ...
            if mF1>F1_best:
                ...
                f.write('Save Model\n') 
                print('Save Model')                     
                model_name = 'Vaihingen_batch'+repr(batch_index+1)+'mF1_'+repr(int(mF1*10000))+'.pth'
